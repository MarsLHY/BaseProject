...
+ (NSData *)signData:(NSData *)data privateKey:(NSString *)privKey {
    ...
    return [RSA signWithRSA:data Key:keyRef];
}

+ (NSData*)signWithRSA:(NSData*)plainData Key:(SecKeyRef)Key {
    
    // Get size of Key
    size_t signedHashBytesSize = SecKeyGetBlockSize(Key);
    // C allocate, string key
    uint8_t* signedHashBytes = malloc(signedHashBytesSize);
    
    // copy unsigned Char 0x0, to the first signedHashBytesSize characters of string pointed to, signedHashBytesSize
    memset(signedHashBytes, 0x0, signedHashBytesSize);
    
    // get size of a SHA256 Digest
    size_t hashBytesSize = CC_SHA1_DIGEST_LENGTH;
    // C allocate, string hash
    uint8_t* hashBytes = malloc(hashBytesSize);
    
    // Take data and hash it
    if (!CC_SHA1([plainData bytes], (CC_LONG)[plainData length], hashBytes)) {
        // failed
        return nil;
    }
    
    // run signiture on hashed data, with key
    SecKeyRawSign(Key,
                  kSecPaddingPKCS1SHA1,
                  hashBytes,
                  hashBytesSize,
    ...
    if (!signedHash) {
        NSLog(@"(FAIL) Key Signing Failure.");
        return nil;
    }
    
    NSData *returnData = signedHash;
    signedHash = NULL;
...
+ (BOOL)verifyData:(NSData *)data publicKey:(NSString *)publicKey {
    ...
    return [RSA verifyWithRSA:data Key:keyRef];
}

+ (BOOL)verifyWithRSA:(NSData*)plainData Key:(SecKeyRef)Key {
    
    size_t signedHashBytesSize = SecKeyGetBlockSize(Key);
    const void* signedHashBytes = [plainData bytes];
